 ...
 */
export function saveAndSetup(ctx: CanvasRenderingContext2D, alpha?: number): void {
  ctx.save();
  if (alpha !== undefined) {
 ...
 */
export function restore(ctx: CanvasRenderingContext2D): void {
  ctx.restore();
}
...
export function renderHealthBar(
  ...
  // Background
  fillRect(ctx, x, y, width, height, colors.background, colors.border);
  
  // Health bar
  const healthWidth = width * healthRatio;
  if (healthWidth > 0) {
    fillRect(ctx, x, y, healthWidth, height, colors.health);
  }
  
  // Damaged portion (if specified)
  if (colors.damaged && healthRatio < 1) {
    const damagedWidth = width * (1 - healthRatio);
    fillRect(ctx, x + healthWidth, y, damagedWidth, height, colors.damaged);
  }
}
...
export function renderParticles(
  ...
  particles.forEach(particle => {
    withAlpha(ctx, particle.alpha, (ctx) => {
      fillRect(ctx, particle.x, particle.y, particle.size, particle.size, particle.color);
    });
  });
}
