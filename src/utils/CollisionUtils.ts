 ...
 */
export function wouldCollideHorizontally(
  targetX: number,
  currentY: number,
  objectWidth: number,
  objectHeight: number,
  ...
  const bounds = normalizeBounds(solidBlock.getBounds());
 ...
 */
export function wouldCollideVertically(
  currentX: number,
  targetY: number,
  objectWidth: number,
  objectHeight: number,
  ...
  const bounds = normalizeBounds(platform.getBounds());
 ...
 */
export function checkCollision(obj1: CollidableObject, obj2: CollidableObject): boolean {
  const bounds1 = normalizeBounds(obj1.getBounds());
  const bounds2 = normalizeBounds(obj2.getBounds());
 ...
 */
export function isWithinLevelBounds(
  position: { x: number; y: number },
  size: { x: number; y: number },
  levelWidth: number,
  levelHeight: number,
 ...
 */
export function checkDiagonalPlatformCollision(
  objectX: number,
  objectY: number,
  objectWidth: number,
  objectHeight: number,
  ...
  const bounds = normalizeBounds(diagonalPlatform.getBounds());
 ...
 */
export function getClosestEdgeDistance(
  obj: CollidableObject,
  target: CollidableObject,
): { direction: "left" | "right" | "up" | "down"; distance: number } {
  const bounds1 = normalizeBounds(obj.getBounds());
  const bounds2 = normalizeBounds(target.getBounds());
...
export function resolveCollision(
  ...
  const edge = getClosestEdgeDistance(movingObject, staticObject);

  if (
    preventDirection === "horizontal" &&
